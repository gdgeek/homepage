...
function translateErrorMessage(message, lang) {
    ...
    for (const errorKey in translations) {
        if (lowerMessage.includes(errorKey)) {
            return (translations[errorKey][currentLang] ||
...
function loginUser(username, password) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield fetch(`${API_BASE_URL}/v1/auth/login`, {
                method: "POST",
                headers: {
            ...
            if (!response.ok) {
                ...
                if (response.status === 401) {
                    ...
                    const translatedMsg = translateErrorMessage(data.message || "");
                    throw new Error(translatedMsg || `登录失败 (错误代码: ${response.status})`);
                }
            }
            return data;
        }
